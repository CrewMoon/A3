...
public class Board
{
    ...
    /// </summary>
    /// <param name="move"></param>
    /// <returns>true iff the move is legal</returns>
    public bool checkMove(ChessMove move)
    {
        ...
        if (getBoardPosState(move.getDst()) != BoardPositionState.EMPTY)
        {
            return false;
        }
        else
    ...
    /// <returns> Returns true iff param "who" could move, ignoring whether it is
                ...
                if (getBoardPosState(new Vector2Int(i, j)) == who)
                {
                    for (int m = -2; m <= 2; m++)
                    {
                        for (int n = -2; n <= 2; n++)
                        {
                            if (m != 0 && n != 0)
                            {
                                Vector2Int possibleDst = new Vector2Int(i + m, j + n);

                                if (!Board.isPosInBoard(possibleDst))
                                {
                                    continue;
                                }
    ...
    )
    {
        ...
        if (!checkMove(move))
        {
            throw new System.ArgumentException("The move is illegal or the game has ended.");
        }
        ...
        for (int i = move.getDst().x - 1; i <= move.getDst().x + 1; i++)
        {
            for (int j = move.getDst().y - 1; j <= move.getDst().y + 1; j++)
            {
                ...
                if (getBoardPosState(temp) == opponent)
                {
