...
public class AIChessPlayer : ConcreteChessPlayer
{
    ...
    public override bool hasFinishedMakingMove()
    {
        ...
        // if it the thread has already dead.
        return !aiThread.IsAlive;
    }
    ...
    private ChessMove findMove()
    {
        ...
        lastFoundMove = new ChessMove(getSide(), move.getDst(), move.getDst());

        // Minimax Search
        miniMax(contemplateBoard, difficulty, true, 1, int.MinValue, int.MaxValue);
    ...
    private int miniMax(Board board, int depth, bool saveMove, int sense, int alpha, int beta)
    {
        ...
        if (depth == 0 || board.getBoardState() != Board.BoardState.PLAYING)
        {
            // return the score of the AI
            return board.getBoardStatistics().z - board.getBoardStatistics().y;
        }
    ...
    private ArrayList makeSequenceRandom(ArrayList listOfMoves)
    {
        ...
        {
            int randomIndex = rand.Next(i, listOfMoves.Count);
